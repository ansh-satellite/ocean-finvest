...
def load_and_process_data(file_path):
    ...
    # BUT only if they actually exist in the file and haven't been completely sold (Quantity > 0).
    ...
    is_held = trades["Quantity"] > 0
    trades["Is_Active"] = is_latest_date | (is_permanent & is_held)
...
def build_daily_table(active_holdings, daily_snapshot):
    rows = []
    maxfolio_path = r"C:\Users\LENOVO\Desktop\Ocean Finvest\MOMENTUM_DB_2 copy\Momentum_Maxfolio.xlsx"
    maxfolio_df = None
    import os
...
def build_daily_contribution_table(daily_table):
    ...
    # Fixed asset allocation weights
    weight_map = {
        "Equity": 75.0,
        "Gold": 10.0,
        "Liquidcase": 15.0
            ...
            [
                {
                    ...
                    "PORT": round(float(live_month_return), 2),
                    "BSE 500": round(float(live_month_bm_return), 2),
                    "Alpha": round(float(live_month_return) - float(live_month_bm_return), 2),
                }
            ]
        )
