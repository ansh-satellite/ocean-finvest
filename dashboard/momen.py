...
def get_credentials():
    try:
        ...
        return st.secrets["truedata"]["username"], st.secrets["truedata"]["password"]
    except:
        try:
            s_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "secrets.toml")
...
def fetch_truedata_history(
    ...
    for ticker in ticker_list:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            except Exception as exc:  # catches TypeError/AttributeError from TrueData internals
                last_err = str(exc)
...
if __name__ == "__main__":
    ...
    UNIVERSE_FILE = os.path.join(DATA_DIR, "Nifty_500_2025_Apr.csv")
    MASTER_FILE   = os.path.join(DATA_DIR, "master_momentum_summary.xlsx")
    ...
    # ── C1) Generate NAV.xlsx from Portfolio 1 ────────────────────────────
    logger.info("Generating NAV.xlsx from Portfolio 1...")
    NAV_OUT_PATH = os.path.join(DATA_DIR, "NAV.xlsx")

    # Step 1: Daily total portfolio value → filter from 2025-11-11 → PORT NAV (100-base)
    NAV_INCEPTION = '2025-11-11'
    daily_port = (
        full_history_df
        .groupby('Date')['Buy_Hold_Value']
        .sum()
        .reset_index()
        .sort_values('Date')
        .reset_index(drop=True)
    ...
    daily_port = daily_port[daily_port['Date'] >= NAV_INCEPTION].reset_index(drop=True)
    start_val = daily_port['Buy_Hold_Value'].iloc[0]
    daily_port['PORT NAV'] = (daily_port['Buy_Hold_Value'] / start_val) * 100

    # Step 2: Fetch BSE500 → BM NAV (100-base at 2025-11-11)
    logger.info("Fetching BSE500 benchmark for BM NAV...")
    bse_df, _ = fetch_truedata_history(['BSE500'], duration='10 Y', bar_size='EOD')
    if not bse_df.empty:
        bse_df = (
            bse_df[bse_df['Date'] >= NAV_INCEPTION]
            .sort_values('Date')
            .reset_index(drop=True)
        )
        bse_start = bse_df['Close'].iloc[0]          # BSE500 value on 2025-11-11 = base 100
        bse_df['BM NAV'] = (bse_df['Close'] / bse_start) * 100
        bse_df = bse_df[['Date', 'BM NAV']]
        nav_out = daily_port[['Date', 'PORT NAV']].merge(bse_df, on='Date', how='left')
        nav_out['BM NAV'] = nav_out['BM NAV'].ffill()
    else:
        logger.warning("BSE500 fetch failed — BM NAV will be empty.")
        nav_out = daily_port[['Date', 'PORT NAV']].copy()
        nav_out['BM NAV'] = np.nan

    nav_out = nav_out.rename(columns={'Date': 'DATE'})
    nav_out.to_excel(NAV_OUT_PATH, index=False)
    logger.info(f"NAV.xlsx saved → {NAV_OUT_PATH}  ({len(nav_out)} rows, starting {NAV_INCEPTION})")
